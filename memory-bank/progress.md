...
- [x] **Inversion de l'affichage des paliers de buts dans `#streak-colors-container`.** Le palier critique (rouge) est affiché en premier, puis les paliers moins critiques vers le vert en bas. `renderStreakColors` itère désormais de `n-1` vers `0` (du palier critique vers le palier le plus bas).
- [x] **Rouge fixe au palier critique + ajout de couleurs vers le vert.** `generateStreakColors(n)` prend désormais les n dernières nuances de `COLOR_PALETTE` (étendu à 10 nuances via `generateGradientPalette(10)`). Quand on augmente le nombre de paliers, on ajoute une couleur plus verte en bas, le rouge restant fixe au palier critique.
